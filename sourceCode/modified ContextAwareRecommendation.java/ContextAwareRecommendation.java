...
public class ContextAwareRecommendation {
	...
	public ContextAwareRecommendation(String sourceDir, String suFolder, int numOfNeighbours, int teStartPos,
		...
		testDir = this.srcDir + subFolder + "/" + "TestingInvocations" + "/";
	...
	public byte[][][] buildUserItemContextMatrix(String testingPro, List<String> listOfProjects,
												 List<String> listOfMethodInvocations, List<String> listOfInvocationMethods) {
		...
		//System.out.println("**********************************************************     listOfPRs's sieze 1: " + listOfPRs.size());


		// The slice for the testing project is located at the end of the matrix
		listOfPRs.add(testingPro);
		//System.out.println("**********************************************************     listOfPRs's sieze 2: " + listOfPRs.size());
		...
		for (String l : listOfMDs)
			listOfInvocationMethods.add(l);
	...
	public void recommendation() {
		...
		for (String testingPro : testingProjects.values()) {
			...
			List<String> listOfMDs = new ArrayList<>();

			Map<String, Float> simScores = reader.getSimilarityScores(simDir + testingPro, numOfNeighbours);

			byte matrix[][][] = buildUserItemContextMatrix(testingPro, listOfPRs, listOfMIs, listOfMDs);
			...
			reader.writeResults(testDir+"simD"+testingPro,simSortedMap,listOfMDs);
	...
	private boolean sotreResults(Map<String, Float> mdSimScores){
		return true;
	}
}
